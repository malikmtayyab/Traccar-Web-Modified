...
import { ReactComponent as GeofenceIcon } from '../resources/images/icon/geo-fence.svg';
...
const DashboardWidget = ({ deviceWithGeofencingViolation, geofencingViolation, device, progress }) => {
                ...
                <>
                    <div className={classes.circle}>
                        <Circle
                            percent={
                                device > 0
                                    ? (deviceWithGeofencingViolation / device) * 100
                                    : 0
                            }
                            trailWidth={9}
                            trailColor="#EDF4FB"
                        ...
                        <h3 className={classes.percent}>
                            {device > 0
                                ? Math.round((deviceWithGeofencingViolation / device) * 100)
                                : 0}
                            %
                        </h3>
                    </div>
                    <div className="card-small-rowRight">
                        <Typography className={classes.cardSmallText}>
                            <span className="card-small-text">
                                {t('sharedGeofenceViolation')}
                            </span>
                        </Typography>
                        <div className={classes.divalignedhorizontal}>
                            <div>
                                <Tooltip title={valueDescText}>
                                    <span className="card-small-value">{geofencingViolation}</span>
                                </Tooltip>
                            </div>
                            <div className="card-small-value">
                                <GeofenceIcon width={16} height={16} />
                            </div>
                        </div>
                        <div className={classes.reportFooter}>
                            ...
                            <Typography
                                ...
                                style={{ color: '#FFA031', fontWeight: 500, paddingLeft: 10 }}
                                variant="caption"
                            >
                                {deviceWithGeofencingViolation}
                            </Typography>
                        </div>
                    </div>
                </>
...
const ChartGeofenceViolation = ({ deviceWithGeofencingViolation, geofencingViolation, device, progress }) => {
    const classes = useStyles();
    return (
        <Card className={classes.reportCard}>
            <CardContent>
                <Grid container spacing={1} sx={{ justifyContent: 'space-between' }}>
                    <DashboardWidget
                        progress={progress}
                        deviceWithGeofencingViolation={deviceWithGeofencingViolation}
                        geofencingViolation={geofencingViolation}
                        device={device}
                    />
                </Grid>
            </CardContent>
...
/* eslint-enable */
export default ChartGeofenceViolation;
